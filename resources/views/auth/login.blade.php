...
<div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0">
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg border border-gray-100">
        <h2 class="text-center text-2xl font-black text-red-600 mb-6 uppercase">Connexion SauveVie</h2>

        <form method="POST" action="{{ route('login') }}">
            @csrf
            <div>
                <label class="block font-bold text-sm text-gray-700">Email</label>
                <input type="email" name="email" required autofocus class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
            </div>
